...
export const appTransferMode: ToolcraftTransferMode = {
  animationIntent: {
    ...
    mode: "autonomous",
    reason:
      "The product uses decorative page, modal, status, and escrow rail motion only. There is no user-facing playback, scrub, duration, loop, or video export behavior.",
  },
  mode: "new-toolcraft-app",
};
...
export const appProductReadiness: ToolcraftProductReadiness = {
  ...
  productSummary:
    "A mock-data Toolcraft prototype for redesigning the full Baseshire Hethaway escrow marketplace surface.",
  requestedBehavior:
    "Render all current user and admin pages plus modal workflows as an editorial fintech escrow terminal with brand typography, vector asset systems, mobile bottom dock navigation, restrained autonomous motion, runtime controls, persistence, and PNG export.",
};
...
function makeControlAcceptance(
  ...
  const entry: ToolcraftComponentAcceptance = {
    automated: true,
    automatedTestName: `${target} is covered by schema and renderer contract`,
    browser: true,
    browserTestName,
    componentType,
    evidence: "viewport-side-effect",
    expectedObservable,
    fixture: "Baseshire redesign mock data",
    id: target,
    kind: "control",
    target,
    userAction: `Change ${target} and verify the prototype reflects the selected state.`,
  };

  if (componentType === "select" || componentType === "segmented") {
    return {
      ...entry,
...
export const appAcceptance: readonly ToolcraftComponentAcceptance[] = [
  makeControlAcceptance(
    "prototype.screen",
    "select",
    "browser: prototype screen selector changes visible screen",
    "Changing Screen swaps the visible full-page Baseshire prototype.",
  ),
  makeControlAcceptance(
    "prototype.modal",
    "select",
    "browser: prototype modal selector changes workflow overlay",
    "Changing Modal shows the selected workflow overlay and closing it returns to the page.",
  ),
  makeControlAcceptance(
    "prototype.device",
    "segmented",
    "browser: prototype device control changes viewport shell",
    "Changing Device constrains the prototype to phone or desktop layout without overlap.",
  ),
  makeControlAcceptance(
    "prototype.density",
    "segmented",
    "browser: prototype density control changes spacing",
    "Changing Density tightens or relaxes card and page spacing.",
  ),
  makeControlAcceptance(
    "prototype.role",
    "segmented",
    "browser: prototype role control changes trust context",
    "Changing Role updates visible buyer, seller, or admin trust context.",
  ),
  makeControlAcceptance(
    "appearance.visualMode",
    "select",
    "browser: prototype visual mode control changes product skin",
    "Changing Visual mode switches the terminal, editorial, or boardroom skin while preserving the same mock data.",
  ),
  makeControlAcceptance(
    "prototype.motion",
    "select",
    "browser: prototype motion control can reduce animation",
    "Changing Motion can reduce decorative page, modal, rail, and status animation while preserving content.",
  ),
  makeControlAcceptance(
    "prototype.trustLevel",
    "select",
    "browser: prototype trust control changes notices",
    "Changing Trust updates standard, enhanced, or dispute notices and badges.",
  ),
  makeControlAcceptance(
    "motion.pulse",
    "slider",
    "browser: rail pulse slider changes animation intensity",
    "Dragging Pulse adjusts the animated escrow rail intensity without adding playback controls.",
  ),
  makeControlAcceptance(
    "appearance.accent",
    "color",
    "browser: prototype accent color changes product output",
    "Changing Royal blue updates navigation, buttons, badges, and escrow rails.",
  ),
  makeControlAcceptance(
    "appearance.background",
    "color",
    "browser: prototype background color changes preview and export",
    "Changing Background updates the live preview product background and PNG export background.",
  ),
  {
    ...makeControlAcceptance(
      "export.includeBackground",
      "switch",
      "browser: prototype background include controls png transparency",
      "Turning Include off hides the live preview product background, makes PNG output transparent, and video output would keep the product background if video export is later enabled.",
    ),
    userAction:
      "Toggle Include off, verify the preview has no product background, export PNG with alpha transparency, and verify video output keeps the background if the product later adds video export.",
  },
  makeControlAcceptance(
    "export.image.format",
    "select",
    "browser: prototype image format control changes export format",
    "Changing Format selects PNG or JPG for the exported prototype image.",
  ),
  makeControlAcceptance(
    "export.image.resolution",
    "select",
    "browser: prototype image resolution exports selected dimensions",
    "Changing Resolution changes the exported image long edge to 2K, 4K, or 8K.",
  ),
  {
    actionCoverage: ["export.png"],
    automated: true,
    automatedTestName: "footer export action renders prototype png",
    browser: true,
    browserTestName: "browser: prototype exports png output",
    componentType: "panelActions",
    evidence: "exported-bytes",
    expectedObservable:
      "The sticky footer Export PNG action downloads a non-empty image rendered from the active prototype state.",
    fixture: "Baseshire redesign export fixture",
    id: "export.actions",
    kind: "control",
    target: "export.actions",
    userAction:
      "Click Export PNG and verify a downloaded image is created from the current prototype screen.",
  },
  {
    automated: true,
    automatedTestName: "all prototype screens render primary entities",
    browser: true,
    browserTestName: "browser: all prototype screens render primary entities",
    componentType: "custom-renderer",
    evidence: "viewport-side-effect",
    expectedObservable:
      "Every user and admin screen renders its primary mock page entities in the shared shell.",
    fixture: "Baseshire full surface fixture",
    id: "prototype.screens",
    kind: "runtime",
    target: "prototype.screens",
    userAction:
      "Switch through every screen and verify page headers, cards, tables, or empty states are visible.",
  },
  {
    automated: true,
    automatedTestName: "all prototype modal workflows render",
    browser: true,
    browserTestName: "browser: all prototype modal workflows render",
    componentType: "custom-renderer",
    evidence: "viewport-side-effect",
    expectedObservable:
      "Every modal workflow renders the shared modal shell with realistic mock content.",
    fixture: "Baseshire modal workflow fixture",
    id: "prototype.modals",
    kind: "runtime",
    target: "prototype.modals",
    userAction:
      "Select every modal workflow and verify the overlay opens, then closes back to the page.",
  },
  {
    automated: true,
    automatedTestName: "prototype settings persist after reload",
    browser: true,
    browserTestName: "browser: prototype settings persist after reload",
    componentType: "runtime",
    evidence: "persistence-state",
    expectedObservable:
      "A changed screen, modal, device, density, role, motion, trust, accent, and background setting restores after a real reload.",
    fixture: "Baseshire persistence fixture",
    id: "prototype.persistence",
    kind: "runtime",
    persistenceCoverage: "reload",
    target: "prototype.persistence",
    userAction:
      "Change persisted prototype controls, reload the browser, and verify the selected state is restored.",
  },
  {
    automated: true,
    automatedTestName: "prototype has no emoji characters",
    browser: true,
    browserTestName: "browser: prototype renders icons without emoji",
    componentType: "custom-renderer",
    evidence: "viewport-side-effect",
    expectedObservable:
      "Navigation, actions, statuses, and empty states render icon elements and no emoji characters appear in product text.",
    fixture: "Baseshire icon-only fixture",
    id: "prototype.icons",
    kind: "runtime",
    target: "prototype.icons",
    userAction:
      "Inspect visible product text and icon-bearing elements across the default page.",
  },
];

export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Prototype review state",
    groupingReason:
      "These controls choose the screen, modal, viewport, role, motion, density, and trust condition a designer is reviewing.",
    targets: [
      "prototype.screen",
      "prototype.modal",
      "prototype.device",
  ...
  {
    entity: "Escrow rail motion",
    groupingReason:
      "The pulse slider is isolated because it tunes the animated escrow rail rather than selecting a prototype screen or modal.",
  ...
  },
  {
    entity: "Brand accent",
    groupingReason:
      "The accent color is the single visual identity token used across navigation, buttons, badges, and escrow rails.",
    splitReason:
      "Accent and background are split because brand identity and export transparency are reviewed in different workflow steps.",
    targets: ["appearance.accent"],
    title: "Brand Accent",
    workflowStage: "Visual review",
  },
  {
    entity: "Output background",
    groupingReason:
      "The include switch and background color together own preview background visibility and PNG transparency.",
    splitReason:
      "Background stays directly before image export because PNG transparency is part of export review, not brand-color review.",
    targets: ["export.includeBackground", "appearance.background"],
    title: "Background",
    workflowStage: "Export preparation",
  },
  {
    workflowStage: "Image export",
    groupingReason:
      "Format and resolution are paired output settings applied directly before the sticky PNG export action.",
...
export function validateToolcraftAcceptanceCoverage(
  ...
  const errors: string[] = [];
  const usesDefaultSectionInventory =
    sectionInventory === starterControlSectionInventory;
  const effectiveSectionInventory =
    schema === appSchema || !usesDefaultSectionInventory
      ? sectionInventory
      : [];
  const usesDefaultTransferMode = transferMode === appTransferMode;
  const effectiveTransferMode =
    schema === appSchema || !usesDefaultTransferMode
      ? transferMode
      : ({ mode: "new-toolcraft-app" } satisfies ToolcraftTransferMode);
  const controls = collectToolcraftVisibleControls(schema);
  const controlTargetsForSchema = new Set(controls.map(({ control }) => control.target));
  const acceptance =
    schema === appSchema
      ? rawAcceptance
      : rawAcceptance.filter(
          (entry) =>
            entry.kind !== "control" ||
            !entry.target ||
            controlTargetsForSchema.has(entry.target),
        );
