...
import { MINI_APPS } from "@/lib/data";
...
export default function MiniAppsPage() {
  ...
  const kinds = ["all", ...new Set(MINI_APPS.map(a => a.kind))];
      ...
      <div className="row between" style={{ alignItems: "center", marginBottom: 22, gap: 24, rowGap: 16, flexWrap: "wrap" }}>
        ...
        <div className="row" style={{ gap: 18, alignItems: "center", flex: "0 0 auto", flexWrap: "wrap" }}>
          <div className="col right" style={{ gap: 1 }}>
            ...
            <span className="mono" style={{ fontSize: 14 }}>{MINI_APPS.length}</span>
          </div>
          <div className="col right" style={{ gap: 1 }}>
            <span className="smallcaps">In escrow</span>
            <span className="mono" style={{ fontSize: 14 }}>226.0 Ξ</span>
          </div>
        </div>
      </div>
          ...
          <div className="chips">
            {kinds.map(k => (
              <button key={k} className={"chip" + (filter === k ? " active" : "")} onClick={() => setFilter(k)}>
                {k === "all" ? "All" : k} <span className="count">{k === "all" ? MINI_APPS.length : MINI_APPS.filter(a => a.kind === k).length}</span>
              </button>
            ))}
          </div>
          <div style={{ flex: 1 }} />
        ...
        {filt.map(a => (
          <Link href="/deals" key={a.id} className="loan-card">
            <div style={{ position: "relative", aspectRatio: "16/10", background: `linear-gradient(135deg, ${appColor(a.id, 0)}, ${appColor(a.id, 1)})`, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
              ...
              <span className="pill" style={{ position: "absolute", top: 10, left: 10, background: "rgba(0,0,0,0.45)", borderColor: "transparent" }}>
                <span className="pdot" style={{ background: a.verified ? "var(--gold)" : "var(--ink-4)" }}/>{a.kind}
              </span>
            ...
            <div className="row between">
              <span className="nm trunc">{a.name}</span>
              <span className="mono" style={{ fontSize: 11, color: "var(--ink-4)" }}>{a.id}</span>
            </div>
            <div className="row between">
              <Stat lab="DAU" v={fmtCompact(a.dau)} />
              <Stat lab="MRR" v={`${a.mrr} Ξ`} />
              <Stat lab="Age" v={a.age} />
            </div>
            <div>
              <div className="row" style={{ gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                {a.stack.map(s => <span key={s} className="chip" style={{ pointerEvents: "none", padding: "2px 7px", fontSize: 10.5 }}>{s}</span>)}
              </div>
