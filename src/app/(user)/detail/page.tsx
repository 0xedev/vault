...
function AcceptLoanModal({ onClose }: { onClose: () => void }) {
  const [confirm, setConfirm] = useState("");
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        <div className="modal-h">
          <h3 className="serif" style={{ margin: 0, fontSize: 22 }}>Fund loan · 8.400 Ξ</h3>
          <button className="btn ghost sm" onClick={onClose}><Icon.x /></button>
        </div>
        <div className="modal-b">
          <div className="warn-banner" style={{ marginBottom: 16 }}>
            <Icon.warn />
            <div>8.400 Ξ will leave your wallet and be locked in escrow until repayment or default.</div>
          </div>
          <div className="kv"><span className="k">You send</span><span className="v big">8.400 Ξ</span></div>
          <div className="kv"><span className="k">You receive at repayment</span><span className="v" style={{ color: "var(--accent)" }}>8.498 Ξ</span></div>
          <div className="kv"><span className="k">If borrower defaults</span><span className="v">Meridian Genesis #1147</span></div>
          <div className="kv"><span className="k">Term</span><span className="v">30 days · matures Apr 30 22:00 UTC</span></div>
          <div className="kv"><span className="k">Platform fee</span><span className="v">0.126 Ξ (1.5%)</span></div>
...
function CounterOfferModal({ onClose }: { onClose: () => void }) {
  const [amt, setAmt] = useState(8.0);
  const [apr, setApr] = useState(13.0);
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-b">
          <p className="muted" style={{ marginTop: 0, fontSize: 13 }}>Borrower listed 8.4 Ξ at 14.2% / 30d. Submit your terms — borrower can accept, reject, or counter back.</p>
          ...
          <div className="kv"><span className="k">Implied LTV</span><span className="v">{Math.round(amt / 15.0 * 100)}%</span></div>
  ...
  return (
    <main className="main">
      <div className="row" style={{ marginBottom: 14, gap: 8, fontSize: 12, color: "var(--ink-4)" }}>
        ...
        <span>/</span>
        <span className="mono" style={{ color: "var(--ink-2)" }}>{l.id}</span>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1.2fr 1fr", gap: 28 }}>
            ...
            <div className="col" style={{ gap: 6 }}>
              ...
              <div className="row" style={{ marginTop: 14, gap: 18 }}>
                <div className="col" style={{ gap: 1 }}>
                  <span className="smallcaps">Borrower</span>
                  <span className="mono" style={{ fontSize: 13 }}>{l.borrower}</span>
                </div>
                <div className="col" style={{ gap: 1 }}>
                  <span className="smallcaps">Reputation</span>
                  <span className="mono" style={{ fontSize: 13 }}>4.8 · 12 loans</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="row" style={{ borderBottom: "1px solid var(--line)" }}>
              {[["offers", "Offer history"], ["history", "Transactions"], ["terms", "Terms"]].map(([k, t]) => (
                <button key={k} className={"side-link" + (tab === k ? " active" : "")} onClick={() => setTab(k)}
                  style={{ borderRadius: 0, border: 0, borderBottom: tab === k ? "1px solid var(--accent)" : "1px solid transparent", flex: "0 0 auto", padding: "14px 18px", margin: "-1px 0 0", color: tab === k ? "var(--ink)" : "var(--ink-3)", background: "transparent" }}>
                  {t}
                </button>
              ))}
            </div>
        ...
        <div className="col" style={{ gap: 18 }}>
          <div className="card" style={{ padding: 22 }}>
            ...
            <div className="kv"><span className="k">Platform fee</span><span className="v">1.5% · 0.126 Ξ</span></div>
            <div className="kv"><span className="k">Escrow</span><span className="v" style={{ color: "var(--accent)" }}>vault.eth · 4-of-7</span></div>
            ...
            <div className="muted-2" style={{ fontSize: 11.5, marginTop: 10, textAlign: "center" }}>
              By accepting, {fmtETH(l.amt)} Ξ leaves your wallet and is sent to escrow. Funds release at repayment.
            </div>
          </div>
          ...
          <div className="card" style={{ padding: 22 }}>
            <div className="row between" style={{ marginBottom: 12 }}>
              ...
              <span className="pill warn"><span className="pdot" />30 days</span>
            </div>
            ...
            <div className="muted-2" style={{ fontSize: 12, marginTop: 4 }}>Until Apr 30, 22:00 UTC. After this point, collateral becomes claimable by the lender.</div>
            <hr className="hr" style={{ margin: "16px 0" }} />
            <div className="tline">
              <div className="ev done"><div className="ttl">NFT locked in escrow</div><div className="sub">Apr 1, 14:22</div></div>
              <div className="ev done"><div className="ttl">Loan funded · 8.4 Ξ released to borrower</div><div className="sub">Apr 1, 14:24</div></div>
      ...
      {modal === "accept" && <AcceptLoanModal onClose={() => setModal(null)} />}
      {modal === "counter" && <CounterOfferModal onClose={() => setModal(null)} />}
    </main>
  );
}
