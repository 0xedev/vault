...
export async function addEscrowProof(req: NextRequest, escrowId: string) {
  ...
  await auth.db`UPDATE escrows SET stage = 'asset_transferred', updated_at = NOW() WHERE id = ${escrowId} AND stage IN ('funds_locked', 'asset_transferred')`;
...
export async function releaseEscrow(req: NextRequest, escrowId: string) {
  ...
  await auth.db`UPDATE escrows SET stage = 'released', tx_hash = COALESCE(${body.data.txHash || null}, tx_hash), tx_status = CASE WHEN ${body.data.txHash || null} IS NULL THEN tx_status ELSE 'pending' END, updated_at = NOW() WHERE id = ${escrowId}`;
  await auth.db`INSERT INTO transactions (id, escrow_id, from_address, to_address, amount, currency, tx_type, tx_hash, status)
    SELECT ${`T-${Date.now()}`}, id, buyer_address, seller_address, amount, currency, 'escrow_released', ${body.data.txHash || null}, ${body.data.txHash ? "pending" : "offchain"} FROM escrows WHERE id = ${escrowId}`;
...
export async function refundEscrow(req: NextRequest, escrowId: string) {
  ...
  const rows = await auth.db`SELECT * FROM escrows WHERE id = ${escrowId} AND (seller_address = ${auth.user.address} OR buyer_address = ${auth.user.address}) LIMIT 1` as Record<string, unknown>[];
  if (rows.length === 0) return NextResponse.json({ error: "Escrow not found for this session" }, { status: 404 });
  await auth.db`UPDATE escrows SET stage = 'refunded', tx_hash = COALESCE(${body.data.txHash || null}, tx_hash), tx_status = CASE WHEN ${body.data.txHash || null} IS NULL THEN tx_status ELSE 'pending' END, updated_at = NOW() WHERE id = ${escrowId}`;
...
export async function disputeEscrow(req: NextRequest, escrowId: string) {
  const auth = await requireUser(req);
  if ("response" in auth) return auth.response;
  const body = z.object({ reason: z.string().min(10).max(500) }).safeParse(await req.json());
...
export async function resolveDispute(req: NextRequest, disputeId: string) {
  ...
  await auth.db`UPDATE escrows SET stage = ${stage}, tx_hash = COALESCE(${parsed.data.txHash || null}, tx_hash), tx_status = CASE WHEN ${parsed.data.txHash || null} IS NULL THEN tx_status ELSE 'pending' END, updated_at = NOW() WHERE id = ${rows[0].escrow_id}`;
